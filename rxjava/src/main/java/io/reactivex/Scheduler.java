 ...
 * <p>
 * Implementations of the {@link #scheduleDirect} and {@link Worker#schedule} methods are encouraged to call the {@link RxJavaPlugins#onSchedule(Runnable)}
 ...
 * wrap the incoming {@code Runnable} into a class that implements the {@link SchedulerRunnableIntrospection}
 ...
 * 1.负责创建一个Worker对象，来委托Worker调度Runnable。
 * 2.负责把Runnable接口传递给Worker，来负责真正的调度。
 * 3.调度器开始和停止
 * 4.任务开始，停止，延时，取消
 *
 * Scheduler---->Worker---->Dispose
 * Worker实现了Dispose，负责执行Runnable和取消待执行的Runnable，真正的工作者（*****）
 * Scheduler 委托--->Worker执行Runnable
 * Dispose 委托---->Worker取消执行的Runnable
 * 实现接口封装Worker实现功能。
 */
public abstract class Scheduler {
     ...
     * @since 2.0
     * 传入一个Runnable返回一个Disposable对象，来给执行者一个取消的机会。（*****）
     */
    @NonNull
    public Disposable scheduleDirect(@NonNull Runnable run, long delay, @NonNull TimeUnit unit) {
        //Scheduler创建一个Worker，把调度工作，交给Worker来负责具体的调度操作
        final Worker w = createWorker();
        //装饰Runnable
        final Runnable decoratedRun = RxJavaPlugins.onSchedule(run);

        //妙处：DisposeTask封装了Worker，Worker负责接收DisposeTask来执行（*****）
        //创建一个可取消的Runnable类型（装饰模式）
     ...
     * 1.Worker作为Scheduler内部类，替Scheduler完成一部分工作。
     * 2.实现Disposable接口，增加取消功能。
     * 3.提供抽象方法schedule（Runnable）负责调度Runnable
     * 4.负责调度（执行）Runnable和取消执行Runnable(******)
     */
    ...
    /**
     * 1.实现了Runnable封装了Runnable，在run（）方法中调用decoratedRun的run（）方法
     * 2.实现了Disposable封装了Worker，负责dispose（）
     * 3.封装了要执行的任务decoratedRun，以及执行decoratedRun任务的Worker。
     * 4.通过DispoaseTask封装decoratedRun，便于在DisposeTask中控制decoratedRun，得知decoratedRun执行完毕了。
     *
     */
    static final class DisposeTask implements Disposable, Runnable, SchedulerRunnableIntrospection {
        ...
        public void run() {
            ...
            try {
                //在DisposeTask中执行封装的任务decoratedRun，然后执行完decoratedRun后dispose
                decoratedRun.run();
            } finally {
                dispose();
                runner = null;
        ...
        @Override
        public void dispose() {
            //封装了Worker就是在任务执行完毕后通过worker来dispose（）；
